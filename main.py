...
def get_recommendation_svd(books, ratings_data, user_id, n=5):
    ...
    if n <= len(next_books):
        ...
        ratings = sorted(ratings, key=lambda x: x[1], reverse=True)[:n]
        book_ids = [id_ for id_, rate in ratings]
...
def main():
    ...
    with st.expander("See explanation"):
        st.write("""
            ...
            4. Collabrative Filtering
            This approach recommends books to users based on their prior reading habits and the preferences of other users.
            It requires a collection of items that relies on the user’s previous selections.The responses of other users are 
            taken into account when recommending a specific product to the main user.
